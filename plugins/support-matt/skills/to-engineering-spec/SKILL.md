...
**但也不要重問已經問過的事。** grilling 與 Matt Spec 已經確認的需求、範圍與行為，直接引用，不重新訪談；程式碼能回答的問題自己查程式碼。反問只留給「缺少、且會影響需求或重大設計」的項目。
...
3. 使用者裁決後，只改那個事實的權威位置：屬於需求／範圍／行為的，回 Matt Spec 改；屬於系統分析／設計決策／實作約束／驗證條件的，改本文件；屬於重大且難以回復的取捨，補 ADR。**注意分界**：需求要不要做在 Matt Spec，怎麼驗證它成立在本文件的「驗證條件」章——後者改動不必回 Matt Spec，前者不得在本文件夾帶。
...
7. 使用者確認後：寫**設計段**——回填各 BR 的 `實作約束`、新增 模組責任、介面契約、資料與 Schema、交易與併發、相容性與風險、驗證條件、交給 to-tickets。`文件狀態` 更新為 `設計中` → `可拆 Ticket`。
8. 停止。輸出 handoff（見下方）。
...
6. **收斂驗證條件**——這是定稿模式最容易漏、漏了下游全錯的一步。掃過本 branch 全部 Ticket，逐條處理三種情況：
   - 開發中新增的可觀察行為（含 Ticket 裡才長出來的邊界值與錯誤訊息）→ 補進「驗證條件」章。
   - 後開的票**推翻或改寫**了先前的行為 → 就地改掉那條 `VC-xx`，不新增第二條並存。
   - 功能被移除 → 刪掉對應的 `VC-xx`，並在「修訂紀錄」記一列。
   Ticket 本身**不回頭修改**；它是切片當下的快照，收斂發生在本文件。
7. `文件狀態` 更新為 `定稿`。停止並告知使用者可用 `engineering-spec-deliverable` 產出交付版貼回公司 GitLab Issue，並可用 `to-acceptance-map` 盤點驗證條件的自動化測試覆蓋。
...
**Matt Spec 決定「要不要做、做到什麼程度」；本章決定「怎麼驗證它成立」。** 不重寫 user stories，不擴充也不縮減需求範圍——每條都必須在「依據」欄指回 user story、BR 或本文件的設計章節。**指不回去的是新需求**，停下來請使用者先改 Matt Spec。

驗證條件的家在本文件而不是散在各 Ticket，理由是**時態**：本文件是這條管線裡唯一會就地修訂的那一份。Ticket 的驗收條件是切片當下的快照，後開的票可能砍掉或改寫前面票的行為，舊票不會回頭更新——拿一疊快照當驗證基準，越到後期錯得越多。

### 粒度（本章唯一的硬要求）

**一條必須可判斷真偽**：明確的前提 + 可觀察的結果，含錯誤訊息原文、邊界值、排序依據、預設值。「系統應正確處理匯出」判不了真偽，不是驗證條件。粒度一旦上浮，這章就只剩裝飾作用。

### 驗證方式

`自動化測試` / `人工檢視` / `靜態檢查`。**護欄：使用者觀察得到的行為一律填 `自動化測試`**，只有純視覺版面、需要真實外部環境而專案未建置該層測試、或約束實作路徑而非行為的，才填其他兩種。判斷不了就填 `自動化測試`。

填 `靜態檢查` 的多是對實作路徑的約束（「四道判定全部在資料庫執行」「查詢與匯出走同一個取資料的方法」）。**它們是拆票前就要遵守的實作約束，不是等驗收才看的事**——強制力由「設計決策」標記承載，不因為和行為條目同表而降級。

### 防膨脹（合併後最需要守的）

三條排除規則對全表適用，寫入前逐條檢查：**已在 Matt Spec 寫明的**、**只是重述本文件其他章節的**、**能被單一章節完整擁有的**（只影響一條 BR 的，進那條 BR 的實作約束欄），一律不放。

另外兩條：

- **不為每個等價類各寫一條。** 本章不是測試案例清單，那是測試程式的事。
- **填 `靜態檢查` 的超過十條**，通常代表排除規則沒被執行。行為條目沒有這個上限——一個像樣的功能本來就有二十條以上的行為驗證。

一條需要三行才講得清楚，代表它在重述 BR，收斂它。
...
- **Matt Spec** 決定需求與範圍；本文件不覆蓋它。
- **驗證基準的權威是本文件的「驗證條件」章**。Ticket 的驗收條件是切片完成條件，不是驗證基準；開發中發現的新條件回本文件補，不要只留在 Ticket 裡。
...
- 業務規則以小節承載，只寫業務效果；程式落點回模組責任、畫面行為回功能介面、安全實作要求回驗證條件。**不存在「實作注意事項」章節**。
...
- 「驗證條件」每條都可判斷真偽（有前提、有可觀察結果）、有依據編號、有驗證方式；沒有一條是「系統應正確處理 X」這種判不了真偽的句子。
- 「驗證條件」沒有為每個等價類各寫一條；填 `靜態檢查` 的沒有膨脹到十條以上。
- 最後一章是 handoff 給 `to-tickets`，倒數第二章是「驗證條件」。
