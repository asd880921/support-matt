...
- 不寫讀者能自己推導出來的內容（見 `writing-rules.md` 第四章）；推導不明顯但重要的，收斂成一條全域實作約束。
...
>
> BR 編號一經產生即為穩定識別碼（全域實作約束、Ticket、code review 都引用它），**修訂時不重新編號**；分類需調整時給新編號、舊編號標記廢棄。

### BR-01 <規則標題>

<規則本文：這條規則要達成什麼業務效果、在什麼條件下成立。可分段、可條列、可含子表格。

**只寫業務效果。** 程式落點是七、模組責任的事，畫面行為是五、功能介面的事，「不可只靠前端擋」是十三、全域實作約束的事——都不重複寫進這裡。規則若定義為「與 X 相同，除了 Y」，就寫這一句，不要把 X 的內容逐條展開。見 `writing-rules.md` 第四章。>
...
> **驗收基準的權威是 [spec.md](./spec.md)。** 本章不重寫 user stories，只補「全域實作約束」——Matt Spec 表達不出、但設計上必須成立的事。
>
> 一條一句可判斷真偽的結果陳述 + 依據編號。

**本章只收「屬於整體、不屬於任何一章」的約束。** 典型長相：橫跨數條 BR 的一致性要求（「這三處必須共用同一個運算式」）、約束實作路徑而非行為的要求（「四道判定必須在資料庫執行」）、跨模組的收斂要求（「查詢與匯出走同一個取資料的方法」）。這些寫進任何單一章節都不完整，寫進每一章就重複數次——本章是它們唯一的家。

**三條排除規則，寫入前逐條檢查，任一命中就不要放進來：**

1. **已在 `spec.md` 任何章節寫明的，不重複** —— 不限於驗收基準。`spec.md` 的 Testing Decisions、技術澄清等段落也算。那邊寫過的，這裡引用編號即可，不要再寫一次。
2. **只是重述本文件其他章節的，刪** —— 「舊子頁已移除」屬於二、範圍；「本次未產生資料庫異動腳本」屬於資料與 Schema。它們在該章已成立，搬到這裡只是換個地方再說一次。
3. **能被單一章節完整擁有的，寫回那一章** —— 只影響一條 BR 的約束，寫進那條 BR 的實作約束欄；只影響一個模組的，寫進模組責任。本章不是實作要求的集散地。

檢查後若只剩三、五條，那是正常的——**這章短才是健康的**。條目膨脹到十條以上，通常代表上述三條沒被執行。
...
- `角色與權限`：
  - `依賴判斷` 是本章的重點。當授權需要「權限 × 資料歸屬 / 層級 / 狀態」的組合時，**為每個依賴條件取一個具名判斷式**（例如 `CanActAsOriginator`），在此定義一次，後續一律引用名稱。沒有具名判斷式，同一段依賴邏輯會在權限表、動作表、介面契約、全域實作約束各重寫一次。
...
- `全域實作約束`：只收「屬於整體、不屬於任何一章」的約束，**寫入前跑該章的三條排除規則**（已在 `spec.md` 寫過的、只是重述本文件其他章節的、能被單一章節完整擁有的，一律不放）。一條一句 + 依據編號；需要三行才講得清楚，代表它在重述 BR。**安全與正確性的實作要求歸這裡**——「Server 端必須獨立驗權限，不可只靠隱藏按鈕」是全域實作約束，不是 BR。**本章短才是健康的**，膨脹到十條以上通常代表排除規則沒被執行。
...
| 定稿 | 同步已核准的變更、全域實作約束加「驗證結果」欄、必要時追加「偏差與待處理」附錄 | `定稿` |
