...
- 不寫讀者能自己推導出來的內容（見 `writing-rules.md` 第四章）；推導不明顯但重要的，收斂成一條驗證條件。
...
**只寫業務效果。** 程式落點是七、模組責任的事，畫面行為是五、功能介面的事，「不可只靠前端擋」是十三、驗證條件的事——都不重複寫進這裡。規則若定義為「與 X 相同，除了 Y」，就寫這一句，不要把 X 的內容逐條展開。見 `writing-rules.md` 第四章。>
...
2. **只是重述本文件其他章節的，刪** —— 「舊子頁已移除」屬於二、範圍；「本次未產生資料庫異動腳本」屬於資料與 Schema。它們在該章已成立，搬到這裡只是換個地方再說一次。
3. **能被單一章節完整擁有的，寫回那一章** —— 只影響一條 BR 的約束，寫進那條 BR 的實作約束欄；只影響一個模組的，寫進模組責任。
...
- `角色與權限`：
  - `依賴判斷` 是本章的重點。當授權需要「權限 × 資料歸屬 / 層級 / 狀態」的組合時，**為每個依賴條件取一個具名判斷式**（例如 `CanActAsOriginator`），在此定義一次，後續一律引用名稱。沒有具名判斷式，同一段依賴邏輯會在權限表、動作表、介面契約、驗證條件各重寫一次。
...
- `驗證條件`：驗證基準的權威位置，必為正文倒數第二章。**行為與實作約束同表，靠 `驗證方式` 欄區分，不拆兩章**——「觀察得到 vs 觀察不到」是怎麼檢查它的差異，那是欄位的事；拿它當章節邊界只會逼寫作者逐條裁決，而且放錯章沒有任何症狀。**安全與正確性的實作要求歸這裡**——「Server 端必須獨立驗權限，不可只靠隱藏按鈕」是驗證條件，不是 BR。這章的價值全部押在粒度上：可判斷真偽才有用，寫成需求摘要就白寫了。開發中新長出來的邊界值與錯誤訊息，定稿時要從 Ticket 收斂回這裡。
