...
當需求影響 persisted data、schema、SQL、reporting、storage、同步、import / export 或資料生命週期時，於**設計段**讀本文件並做出明確決策。

**本文件管的是「決策要做完」，不是「章節要存在」。** 結論若是「沿用單一既有表、無 schema 變更、無 migration、無新的資料 ownership 決策」，就不開「資料與 Schema」章，一句話寫進文件資訊的 `Schema` 列、模組責任或相關 BR 其中一處（見 `document-template.md` 的「章節存在門檻」）。**但那一句是查完才寫得出來的**——跳過檢查直接寫「無變更」不算。
...
有 schema 變更、migration、索引或關聯設計時，「資料與 Schema」必須涵蓋：
...
| 內容 | 寫在哪 |
| --- | --- |
| 資料長什麼樣、誰是權威、怎麼交付 | 資料與 Schema |
| **單條規則**的交易邊界、原子扣量、檢查順序 | 該條 BR 的「實作約束」 |
| **跨規則**的隔離等級、樂觀 / 悲觀策略、重試次數、鎖順序 | 資料存取與一致性 |

例：「`InventoryStock` 以品號 × 倉庫 × 狀態為一組記一數量」是 Schema；「即時庫存表與流水表須同筆交易同步」是 BR-07 的實作約束；「本模組一律樂觀鎖 + 最多重試 1 次，多表寫入固定依 `Order → Stock → Log` 順序」是「資料存取與一致性」。三者引用彼此的編號與表名，不互相重述。
