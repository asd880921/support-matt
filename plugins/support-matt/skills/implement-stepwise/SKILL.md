...
昂貴層 case 數由 3 進 3 出（併方法不省執行時間，這次沒有可刪的 case）。
（本次沒有刪或併時：「無——本次留下 3 支，分別守 A／B／C；昂貴層家族未達 3 個 case，也沒有往既有參數化測試加列。」）
...
- **「驗證」欄要寫明實際跑了什麼範圍。** 整檔跑完就寫「`XxxTests` 全綠（新增 3 條）」；依第 3 節分流只跑了本次新增的方法，就照實寫「`XxxTests` 只跑本次新增的 3 條，全綠；整檔留待收尾」。**本次是收斂型或動到共用測試基礎設施的 commit 時，要寫出整類別／整專案跑完的結果**——那兩種情況只跑新增的不合規。含糊寫成「測試通過」會讓使用者以為同檔既有測試也驗過了。
- **「Test Consolidation」欄不得省略，也不得只寫「已整理」。** 刪或併了哪幾支、該行為現在由誰守，都要點名——這是使用者唯一能攔下誤刪的時機，寫得含糊等於靜默刪測試。
- **「無」也要付一句代價**：列出本次留下的測試各守什麼，並明講本次新增的測試有沒有讓同構家族達到門檻（昂貴層 3 個 case、便宜層 8 支方法）、有沒有往既有的參數化測試加列。**昂貴層有增減時要寫 case 數，不是方法數**——「3 支併成 1 支」看起來像省了，但 case 數沒變的話執行時間一秒未省，寫成「case 數 3 進 3 出」才看得出來。空手寫「無」不接受——它是最省事又永遠不會被抓到的答案，不給它成本，它就會變成預設值。
- **`git add` 也一起等**。關卡之前不要動暫存區，避免使用者以為只是預覽卻已經動了 git 狀態。
- **只在這裡與收尾之前設關卡。** 不要在實作中途、寫完測試、跑完 typecheck 等處另外停下來徵詢。
- **不附上「要不要繼續」的選項題。** 使用者的答案只有兩種：確認（繼續）或提出調整。
- **不必在這裡判斷這是不是最後一個 commit。** 預告收尾的責任在第 5 節開頭那一停，不依賴事前預估。已經確定沒有下一個時，順帶講一句無妨，但講了不免除那一停。
...
最後一個 commit 的關卡照走，確認提交後**接著走第 5 節開頭那一停**，不要直接開始收尾。

## 5. 收尾

### 收尾開始前先停一次（不得跳過）

一個 commit 提交完成後，**若已經沒有下一個 commit 要做**，停下來報一次並等使用者回應，不要直接開始收尾：

```text
沒有更多 commit 了。接著進收尾：跑受影響範圍的測試、做一次跨層重複檢查、
逐條核對驗收條件並把核對結果寫回 ticket，然後結束。
要補做或調整什麼，現在講——收尾之後的修正會變成另一個 commit。
```

**責任是「確認已經沒有 commit 了」，不是「事前預測哪一個是最後一個」。** 實跑案例：規劃 5 個 commit、實際 4 個做完（第 5 個要動的檔案在 `.git/info/exclude` 裡，沒有東西可提交），agent 在第 4 個關卡並不知道那是最後一個，依規則「不確定就不要宣稱」而沒有預告——**沒有違規，但使用者最後一次插手的機會靜默過去了**。改在這裡確認，就不依賴預估準不準。

- **實際的 commit 數少於原本規劃時，這一停照樣要有。** 它的觸發條件是「沒有下一個 commit」，不是「做滿了規劃的數量」。
- 使用者回「繼續」或等同的確認 → 直接往下跑收尾。**提出要補做** → 就地處理，那份補做同樣走第 4 節的關卡，提交後回到這一停。
- **這是收尾唯一的停頓。** 進了收尾就一路做到回報結束，不要在跑完測試、核對完驗收條件等處再停。
