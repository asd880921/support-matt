...
2. **強制探索 codebase 現況**，包含 `Blocked by` 所列 ticket 已經留下的程式碼。這一步不得跳過——commit 拆分必須建立在真實的程式碼狀態上，不是憑 ticket 文字想像。
...
- **本輪 commit 完成、仍有未完成項目** → 告知剩餘項目數，等使用者指示繼續。
- **checklist 全部完成** → 已於收尾回報完整測試與驗收條件核對結果。提示使用者可清空 context，取下一張 blocker 已滿足的 ticket 重新調用本 skill。
