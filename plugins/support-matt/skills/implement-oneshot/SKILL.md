...
- 每個 commit 完成時，依「邊做邊記」記下本次**留下來**的測試與涵蓋的驗收條件，以及該次 consolidation 刪／併了什麼、該行為現在由誰守；收尾時併入寫回 ticket 的核對表與最終回報。**本 skill 自動提交，使用者看不到過程**——consolidation 若沒寫進回報，就等於靜默刪測試。
